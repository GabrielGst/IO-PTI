...
def main():
    ...
    dif = oObject.real - o
    saveImplot(dif, "Difference between the estimated object and the real object", filename="difference_estimated_real_object", save=True, plot=True)
    ...
    rObject, dsprObject = createObject(nPoints=imShape[1], sampling_rate=1, p=2, k=1, rho_0=0.01)
    dsprObject = dsprObject[:int(imShape[0]),:int(imShape[1])]
    sigmaBis = 0.01 * np.max(im)
